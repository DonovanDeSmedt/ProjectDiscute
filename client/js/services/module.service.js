    ...
    return (function(){
      ...
      const deleteDiscute = function(discute){
        if(discute.author === $rootScope.currentUser.username){
          $http.delete('/discute/'+discute._id).then(function(data){
      ...
      const addComment = function(comment, side, discute, username){
        return $http.put('/discute/comment/'+discute._id, {user: username, comment: comment, side: side})
        .then(function(data){
      ...
      const vote = function(side, discute, username){
        $http.put('/discute/vote/'+discute._id, {user: username, side: side})
        .then(function(data){
          _updateDiscute(discute, data.data.left, data.data.right);
        })
        .catch(function(err){
      ...
      const deleteComment = function(side, comment, indexComment, discute, username){
        if(comment.name === username){
          ...
          return $http.put('/discute/uncomment/'+discute._id, {side: side, id: id})
          .then(function(data){
            _updateDiscute(discute, data.data.left, data.data.right);
            return discute;
          })
